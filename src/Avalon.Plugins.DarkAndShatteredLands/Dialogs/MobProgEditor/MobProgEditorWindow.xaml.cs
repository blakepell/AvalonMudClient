        ...
        void AvalonLuaEditor_TextEntered(object sender, TextCompositionEventArgs e)
        {
            ...
            if (e.Text == " ")
            {
                ...
                if (word == "mob")
                {
                    ...
                    data.Add(new MobProgCompletionData("purge", "Usage: mob purge <argument>\r\nDestroys the argument from the room of the mobile. Without an argument the result is the cleansing of all NPC's and items from the room withthe exception of the mobile itself.  However, mppurge $i will indeedpurge the mobile, but it MUST be the last command the mobile tries todo, otherwise the mud cant reference the acting mobile trying to do thecommands and bad things happen."));
                    ...
                    data.Add(new MobProgCompletionData("vforce", "Usage: mob vforce <vnum> <command>\r\nVFORCE affects all mobiles with the specified vnum in the game world.  This is useful for purging a certain type of NPC from the game (by focing them to purge themselves)."));
