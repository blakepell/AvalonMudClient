...
namespace MoonSharp.Interpreter.Interop.BasicDescriptors
{
    ...
    public abstract class DispatchingUserDataDescriptor : IUserDataDescriptor, IOptimizableDescriptor
    {
        ...
        public void AddMetaMember(string name, IMemberDescriptor desc)
        {
            if (desc != null)
            {
                this.AddMemberTo(m_MetaMembers, name, desc);
            }
        }
        ...
        public void AddDynValue(string name, DynValue value)
        {
            ...
            this.AddMemberTo(m_Members, name, desc);
        }
        ...
        {
            var odesc = desc as IOverloadableMemberDescriptor;
        ...
        {
            var methods = UserData.GetExtensionMethodsByNameAndType(indexName, this.Type);
        ...
        private DynValue MultiDispatchLessThanOrEqual(Script script, object obj)
        {
            ...
            if (comp != null)
            {
                return DynValue.NewCallback(
                    (context, args) =>
                        DynValue.NewBoolean(this.PerformComparison(obj, args[0].ToObject(), args[1].ToObject()) <= 0));
            }
        ...
        private DynValue MultiDispatchEqual(Script script, object obj)
        {
            return DynValue.NewCallback(
                (context, args) =>
                    DynValue.NewBoolean(this.CheckEquality(obj, args[0].ToObject(), args[1].ToObject())));
        }
        ...
        private DynValue TryDispatchToNumber(Script script, object obj)
        {
            foreach (var t in NumericConversions.NumericTypesOrdered)
            {
                ...
                var v = this.DispatchMetaOnMethod(script, obj, name);
                if (v != null)
                {
                    return v;
                }
