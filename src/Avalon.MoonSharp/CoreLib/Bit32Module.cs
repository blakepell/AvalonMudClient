...
namespace MoonSharp.Interpreter.CoreLib
{
    ...
    public class Bit32Module
    {
        ...
        {
            uint accum = ToUInt32(args.AsType(0, funcName, DataType.Number));

            for (int i = 1; i < args.Count; i++)
            {
                uint vv = ToUInt32(args.AsType(i, funcName, DataType.Number));
        ...
        private static void ValidatePosWidth(string func, int argPos, int pos, int width)
        {
            ...
            if (width <= 0)
            {
                throw new ScriptRuntimeException("bad argument #{1} to '{0}' (width must be positive)", func,
                    argPos + 1);
            }
        }
